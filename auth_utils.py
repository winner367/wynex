...
# Convenience wrappers requested
def login_user(email: str, password: str) -> bool:
    return auth_api.login(email, password)

def register_user(email: str, password: str, name: str, role: str = "user") -> bool:
    # Route to the DB-backed create_user if available
    try:
        return auth_api.create_user(email, password, name, UserRole(role))
    except Exception:
        return create_user(email, password, name, role)
